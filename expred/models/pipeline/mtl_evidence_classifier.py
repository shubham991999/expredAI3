...
def train_mtl_evidence_classifier(evidence_classifier: nn.Module,
    ...
    optimizer.zero_grad()
    print()
    print("Training output in each epoch by Evidence Classifier :")
    for epoch in range(start_epoch, epochs):
        ...
        for batch_start in range(0, len(epoch_train_data), batch_size):
            ...
            # print("Targets in evidence classifier:")
            # print(targets)
            ids = [(s.ann_id, s.docid, s.index) for s in batch_elements]
            # print("Class interner in evidence classifier :")
            # print(class_interner)
        ...
        print()
        print("Training loss during training the data ")
        logging.info(f'Epoch {epoch} training loss {epoch_train_loss}')
        print()

        with torch.no_grad():

            # print("Prediction and targets of training data during evaluation-")
            # print()
            ...
            results['train_f1'].append(
                classification_report(epoch_train_truth, epoch_train_hard_pred,
            ...
            # print("Prediction and targets of validation data during evaluation-")
            # print()
            epoch_val_loss, \
            epoch_val_soft_pred, \
            epoch_val_hard_pred, \
            ...
            results['val_f1'].append(
                classification_report(epoch_val_truth, epoch_val_hard_pred,
                                      labels=list(range(len(class_labels))), output_dict=True))
            ...
            print()
            print("Training loss and validation loss during Validation time-")
            logging.info(f'Epoch {epoch} train loss {epoch_train_loss}')
            logging.info(f'Epoch {epoch} train_acc {results["train_acc"][-1]}')
            logging.info(f'Epoch {epoch} train_f1 {results["train_f1"][-1]}')
